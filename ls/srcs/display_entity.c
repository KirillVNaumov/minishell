...
/*   Updated: 2018/11/02 11:16:47 by knaumov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			display_detail(t_node *entity, t_flags *flags)
{
	...
	if (flags->flag_n == 1)
		ft_printf("%d  ", entity->buf.st_uid);
	else if (flags->flag_g == 0)
		ft_printf("%-6s  ", getpwuid(entity->buf.st_uid)->pw_name);
	if (flags->flag_n == 1)
		ft_printf("%d  ", entity->buf.st_gid);
	else
		ft_printf("%-11s", getgrgid(entity->buf.st_gid)->gr_name);
